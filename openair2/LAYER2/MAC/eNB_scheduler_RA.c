  ...
  if (ra->rach_resource_type > 0) {
    ...
    if ((ra->Msg2_frame == frameP) && (ra->Msg2_subframe == subframeP)) {
      ...
      if (!CCE_allocation_infeasible(module_idP, CC_idP, 0, subframeP,
        ...
        dl_config_pdu->dlsch_pdu.dlsch_pdu_rel8.length                                 = 7;
        dl_config_pdu->dlsch_pdu.dlsch_pdu_rel8.pdu_index                              = mac->pdu_index[CC_idP];
        dl_config_pdu->dlsch_pdu.dlsch_pdu_rel8.rnti                                   = ra->RA_rnti;
...
//------------------------------------------------------------------------------
{
  ...
  if (ra->rach_resource_type > 0) {
    ...
    // mpdcch_repetition_count == reps
      ...
      lcid = 0;
      UE_info->UE_sched_ctrl[UE_id].round[CC_idP][ra->harq_pid][TB1] = 0;
      ...
      offset = generate_dlsch_header ((unsigned char *) mac->UE_info.DLSCH_pdu[CC_idP][0][(unsigned char) UE_id].payload[0][TB1], 1,       //num_sdus
      ...
      memcpy ((void *) &mac->UE_info.DLSCH_pdu[CC_idP][0][(unsigned char) UE_id].payload[0][TB1][(unsigned char) offset], &cc[CC_idP].CCCH_pdu.payload[0], ra->msg4_rrc_sdu_length);
      // DL request
      mac->TX_req[CC_idP].sfn_sf = (frameP << 4) + subframeP;
      TX_req = &mac->TX_req[CC_idP].tx_request_body.tx_pdu_list[mac->TX_req[CC_idP].tx_request_body.number_of_pdus];
      ...
      TX_req->segments[0].segment_data = mac->UE_info.DLSCH_pdu[CC_idP][0][(unsigned char) UE_id].payload[0][TB1];
      mac->TX_req[CC_idP].tx_request_body.number_of_pdus++;
      ...
      T (T_ENB_MAC_UE_DL_PDU_WITH_DATA, T_INT (module_idP), T_INT (CC_idP), T_INT (ra->rnti), T_INT (frameP), T_INT (subframeP),
         T_INT (0 /*harq_pid always 0? */ ), T_BUFFER (&mac->UE_info.DLSCH_pdu[CC_idP][0][UE_id].payload[0][TB1], ra->msg4_TBsize));
      trace_pdu (DIRECTION_DOWNLINK, (uint8_t *) mac->UE_info.DLSCH_pdu[CC_idP][0][(unsigned char) UE_id].payload[0][TB1],
    ...
    if ((ra->Msg4_frame == frameP) && (ra->Msg4_subframe == subframeP)) {
      ...
      if (rrc_sdu_length > 0) {
        ...
        else if((cc[CC_idP].mib->message.dl_Bandwidth == 4) || (cc[CC_idP].mib->message.dl_Bandwidth == 5))
        {
          rbg_map[0] = 1;

          for(rbg = 1; rbg < N_RBG; rbg++)
          {
            rbg_map[rbg] = 0;
          }
        }
        ...
        fill_nfapi_dl_dci_1(dl_config_pdu, 4,  // aggregation_level
                             ...
                             ra->msg4_mcs,  // mcs
                             1 - UE_info->UE_template[CC_idP][UE_id].oldNDI[ra->harq_pid][TB1],
                             0, // rv
                             0);  // vrb_flag

        UE_info->UE_template[CC_idP][UE_id].oldNDI[ra->harq_pid][TB1] = 1 - UE_info->UE_template[CC_idP][UE_id].oldNDI[ra->harq_pid][TB1];
        LOG_D(MAC,
        ...
        if (!CCE_allocation_infeasible(module_idP, CC_idP, 1, subframeP,
          ...
          offset = generate_dlsch_header((unsigned char *) mac->UE_info.DLSCH_pdu[CC_idP][0][(unsigned char) UE_id].payload[0][TB1],
                                         num_sdus,  //num_sdus
          ...
          memcpy((void *) &mac->UE_info.DLSCH_pdu[CC_idP][0][(unsigned char)UE_id].payload[0][TB1][(unsigned char)offset],
          ...
          fill_nfapi_dlsch_config(&dl_req_body->dl_config_pdu_list[dl_req_body->number_pdu],
                                  ...
                                  ra->rnti,
                                  0, // resource_allocation_type : format 1A/1B/1D
          ...
          trace_pdu(DIRECTION_DOWNLINK,
                    (uint8_t *) mac->
                    UE_info.DLSCH_pdu[CC_idP][0][(unsigned char)UE_id].payload[0][TB1],
...
check_Msg4_retransmission(module_id_t module_idP, int CC_idP,
  ...
  round = UE_info->UE_sched_ctrl[UE_id].round[CC_idP][ra->harq_pid][TB1];
  vrb_map = cc[CC_idP].vrb_map;
  dl_req = &mac->DL_req[CC_idP];
  dl_req_body = &dl_req->dl_config_request_body;
  dl_config_pdu = &dl_req_body->dl_config_pdu_list[dl_req_body->number_pdu];
    ...
    if (ra->rach_resource_type > 0 && round > 0) {
      ...
      if ((ra->Msg4_frame == frameP)
        ...
        fill_nfapi_dl_dci_1(dl_config_pdu, 4,   // aggregation_level
                             ...
                             allocate_prbs_sub(nb_rb, N_RB_DL, N_RBG, rbg_map),     // resource_block_coding
                             ra->msg4_mcs,  // mcs
                             UE_info->UE_template[CC_idP][UE_id].oldNDI[ra->harq_pid][TB1],
                             round & 3, // rv
                             0);  // vrb_flag
