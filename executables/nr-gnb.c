...
void init_eNB_afterRU(void) {
  ...
  for (inst=0; inst<RC.nb_nr_inst; inst++) {
    ...
    FILE *matlab_gen_file = fopen("/home/eurecom/sakthi/pdsch_mcs28_dci_manual_matlab.dat","r");
    ...
    for(int idx_samp=0; idx_samp<30720*2; idx_samp++) {
      *((int16_t*)(gNB->common_vars.debugBuff+idx_samp))>>=1;
      *(((int16_t*)(gNB->common_vars.debugBuff+idx_samp))+1)>>=1;
    }
