...
int wakeup_tx(PHY_VARS_eNB *eNB,
  ...
  if(!(get_thread_parallel_conf() == PARALLEL_RU_L1_TRX_SPLIT)){
    ...
    if(ru_proc != NULL){
      L1_proc_tx->subframe_rx   = ru_proc->tti_rx;
      L1_proc_tx->frame_rx      = ru_proc->frame_rx;
      L1_proc_tx->subframe_tx   = (ru_proc->tti_rx+(sf_ahead-1))%10;
      L1_proc_tx->frame_tx      = (ru_proc->tti_rx>(9-(sf_ahead-1))) ? (ru_proc->frame_rx+1)&1023 : ru_proc->frame_rx;
...
int wakeup_rx(PHY_VARS_eNB *eNB, RU_t *ru){
    ...
    if (L1_proc_rx->instance_cnt==-1) {
      ...
      L1_proc_rx->subframe_rx  = ru_proc->tti_rx;
      L1_proc_rx->frame_tx     = (ru_proc->tti_rx > (9-sf_ahead)) ? (ru_proc->frame_rx+1)&1023 : ru_proc->frame_rx;
      L1_proc_rx->subframe_tx  = (ru_proc->tti_rx + sf_ahead)%10;
...
int wakeup_rxtx(PHY_VARS_eNB *eNB,
  ...
  if ((ret=pthread_mutex_unlock( &L1_proc->mutex))!=0) {
    ...
    if(first_phy_tx == 0){
      if (wakeup_tx(eNB, ru_proc->frame_rx, ru_proc->tti_rx, (L1_proc->subframe_rx > (9-sf_ahead)) ? (L1_proc->frame_rx+1)&1023 : L1_proc->frame_rx, (L1_proc->subframe_rx + sf_ahead)%10, ru_proc->timestamp_rx + (sf_ahead*fp->samples_per_tti)) < 0) {
...
void *pre_scd_task( void *param ) {
  ...
  while (!oai_exit) {
    ...
    memcpy(&pre_scd_eNB_UE_stats,&UE_info->eNB_UE_stats, sizeof(eNB_UE_STATS)*MAX_NUM_CCs*NUMBER_OF_UE_MAX);
    memcpy(&pre_scd_activeUE, &UE_info->active, sizeof(boolean_t)*NUMBER_OF_UE_MAX);
