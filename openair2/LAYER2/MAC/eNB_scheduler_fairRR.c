...
void select_ul_ue_candidate(
  ...
  for ( UE_id_idx = 0; UE_id_idx < NUMBER_OF_UE_MAX; UE_id_idx++ ) {
      ...
      && (UE_sched_ctl->ul_periodic_timer_exp_flag == TRUE) )
    {
      ...
      if ((UE_info->UE_template[CC_id][UE_id].ul_buffer_info[volte_lcg] > 0) && (bytes_to_schedule > 0)) {
...
void dlsch_scheduler_pre_ue_select_fairRR(
  ...
  uint16_t                       dlsch_ue_max_num[MAX_NUM_CCs] = {0};
  uint16_t                       dlsch_ue_max_num_volte[MAX_NUM_CCs] = {0};
  uint16_t                       dlsch_ue_max_num_normal[MAX_NUM_CCs] = {0};
  uint16_t                       dlsch_ue_num_volte[MAX_NUM_CCs] = {0};
  ...
  for (CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    ...
    if(eNB->volte_dl_cycle[CC_id] != 0){
      dlsch_ue_max_num_volte[CC_id] = (uint16_t)(dlsch_ue_max_num[CC_id] / 2);
      dlsch_ue_max_num_normal[CC_id] = dlsch_ue_max_num[CC_id] - dlsch_ue_max_num_volte[CC_id];
    }
    else{
      dlsch_ue_max_num_normal[CC_id] = dlsch_ue_max_num[CC_id];
    }
    // tmp last_dlsch_ue_id_volte
    last_UE_id[CC_id] = last_dlsch_ue_id_volte[CC_id];
  ...
  for (CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++) {
    ...
    for (index=0; index < dlsch_ue_max_num[CC_id]; index++) {
      UE_id=dl_ue_candidate[CC_id][index];
      if(UE_id==-1){
        break;
      }
      if(end_flag[CC_id] == 1) {
      ...
      for (i = 0; i < UE_info->numactiveCCs[UE_id]; i++) {
        ...
        while (TBS < UE_info->UE_template[pCCid][UE_id].dl_buffer_total) {
          ...
          if (nb_rbs_required[CC_id][UE_id] > N_RB_DL) {
            TBS = get_TBS_DL(eNB_UE_stats->dlsch_mcs[TB1], N_RB_DL);
...
//------------------------------------------------------------------------------
{
  ...
  uint8_t select_tb;
  uint8_t oppose_tb;
  uint8_t first_TB_pdu_create_flg = 0;
  ...
  uint32_t dcch_size = 0;
  uint32_t dcch1_size = 0;
  uint32_t dtch_size = 0;
  ...
  for (CC_id=0; CC_id<MAX_NUM_CCs; CC_id++) {
    ...
    for (i = 0; i < dlsch_ue_select[CC_id].ue_num; i++) {
      ...
      */
      ...
      harq_pid = frame_subframe2_dl_harq_pid(cc->tdd_Config,frameP,subframeP);
      select_tb = ue_sched_ctl->select_tb[CC_id][harq_pid];
      oppose_tb = select_tb ^ 0x1;
      round = ue_sched_ctl->round[CC_id][harq_pid][select_tb];
      ...
      UE_info->eNB_UE_stats[CC_id][UE_id].harq_round[TB1] = ue_sched_ctl->round[CC_id][harq_pid][TB1];
      UE_info->eNB_UE_stats[CC_id][UE_id].harq_round[TB2] = ue_sched_ctl->round[CC_id][harq_pid][TB2];
      ...
      if (round != 8) {
        ...
        TBS =
          get_TBS_DL(UE_info->
                     UE_template[CC_id][UE_id].oldmcs[harq_pid][select_tb],
                     nb_rb);
        ...
        if (nb_rb <= nb_available_rb) {
          ...
          //TBS set
          UE_info->eNB_UE_stats[CC_id][UE_id].TBS[select_tb] = TBS;

          //CW_num set
          ue_sched_ctl->cw_num[CC_id][harq_pid] = SINGLE_CW;

          //swap_flg set
          ue_sched_ctl->swap_flag[CC_id][harq_pid] = select_tb;

          if(ue_sched_ctl->aperiodic_ri_received[CC_id] == MULTI_RI){
            //2nd TB start

            if(ue_sched_ctl->round[CC_id][harq_pid][oppose_tb] == 8){
                //new SDU
                sdu_length_total = 0;
                num_sdus         = 0;
                ...
                //GET TBS
                TBS = get_TBS_DL(eNB_UE_stats->dlsch_mcs[oppose_tb], nb_rb);
            ...
            dtch_max_num = UE_info->UE_template[CC_id][UE_id].dl_dtch_num;
            for (dtch = 0; dtch < dtch_max_num; dtch++) {
                lcid = UE_info->UE_template[CC_id][UE_id].dl_dtch_list_priority[dtch];
                header_len_dtch     += 3;
            ...
            if (header_len_dtch == 0){
                ...
                if ((sdu_length_total + header_len_dcch + header_len_dtch) > 0) {
                    ...
                    if (header_len_dtch == 0) {
                        ...
                        header_len_dtch = (header_len_dtch > 0) ? header_len_dtch - header_len_dtch_last : header_len_dtch;
                    }

                    //GET 2nd TB MCS
                    mcs = eNB_UE_stats->dlsch_mcs[oppose_tb];
                    ...
                    if ((TBS - header_len_dcch - header_len_dtch - sdu_length_total - ta_len) <= 2) {
                        ...
                        post_padding = TBS - sdu_length_total - header_len_dcch - header_len_dtch - ta_len;
                    }

                    offset = generate_dlsch_header( (unsigned char *) UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][oppose_tb],
                                                     num_sdus,
                                                     sdu_lengths,   //
                                                     sdu_lcids,
                                                     255,           // no drx
                    ...
                    memcpy(&UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][oppose_tb][offset],dlsch_buffer,sdu_length_total);
                    // memcpy(RC.mac[0].DLSCH_pdu[0][0].payload[0][offset],dcch_buffer,sdu_lengths[0]);

                    // fill remainder of DLSCH with random data
                    for (j=0; j<(TBS-sdu_length_total-offset); j++) {
                        UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][oppose_tb][offset+sdu_length_total+j] = (char)(taus()&0xff);
                    }
                    ...
                    UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[oppose_tb]       = mcs;
                    UE_info->eNB_UE_stats[CC_id][UE_id].TBS[oppose_tb]             = TBS;
                    UE_info->eNB_UE_stats[CC_id][UE_id].overhead_bytes[oppose_tb]  = TBS- sdu_length_total;
                    ...
                    //CW_num set
                    ue_sched_ctl->cw_num[CC_id][harq_pid] = MULTI_CW;
                ...
                //GET TBS
                TBS = get_TBS_DL(UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][oppose_tb], nb_rb);
                UE_info->eNB_UE_stats[CC_id][UE_id].TBS[oppose_tb] = TBS;
        ...
        //2nd TB end
          ...
          case 3:
              ...
              if((UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][select_tb] == 0) && ((UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][select_tb] & 3) == 1)){
                  UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][select_tb]++;
              }

              if((UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][oppose_tb] == 0) && ((UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][oppose_tb] & 3) == 1)){
                  UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][oppose_tb]++;
              }
              ...
              dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.precoding_information                 = 0;
              dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.transport_block_to_codeword_swap_flag = 0;

              if(ue_sched_ctl->cw_num[CC_id][harq_pid] == MULTI_CW){
                  if(select_tb == TB1){
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_1          = UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB1];
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_1                         = UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB1];
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.redundancy_version_1          = UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][TB1] & 3;

                      if(ue_sched_ctl->round[CC_id][harq_pid][oppose_tb] == 8){
                          //2nd TB new SDU
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_2      = 1 - UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB2];
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_2                     = UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[TB2];
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.redundancy_version_2      = 0;
                      }else{
                          //2nd TB RETX
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_2      = UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB2];
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_2                     = UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB2];
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.redundancy_version_2      = UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][TB2] & 3;
                      }
                  }else{
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_2          = UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB2];
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_2                         = UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB2];
                      ...
                      if(ue_sched_ctl->round[CC_id][harq_pid][oppose_tb] == 8){
                          //2nd TB new SDU
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_1      = 1 - UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB1];
                          dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_1                     = UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[TB1];
                  ...
                  if(select_tb == TB1){
                      ...
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_1                         = UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB1];
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.redundancy_version_1          = UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][TB1] & 3;
                      dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.new_data_indicator_2          = 0;
              ...
              if (cc[CC_id].tdd_Config != NULL) {         //TDD
                  ...
                  LOG_D(MAC,
                        ...
                        UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB1]
                        );
              } else {
                  LOG_D(MAC,
              ...
              if (!CCE_allocation_infeasible(module_idP, CC_id, 1, subframeP, dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.aggregation_level, rnti)) {
                  ...
                  fill_nfapi_dlsch_config( &dl_req->dl_config_pdu_list[dl_req->number_pdu],
                                           UE_info->eNB_UE_stats[CC_id][UE_id].TBS[select_tb],
                                           -1,        /* retransmission, no pdu_index */
                                           rnti,
                                           0,         // type 0 allocation from 7.1.6 in 36.213
                                           0,         // virtual_resource_block_assignment_flag, unused here
                                           0,         // resource_block_coding, to be filled in later
                                           getQm(UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][select_tb]),
                                           UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][select_tb] & 3, // redundancy version
                                           (select_tb + 1),   // transport blocks
                                           ue_sched_ctl->swap_flag[CC_id][harq_pid],  // transport block to codeword swap flag
                                           ue_sched_ctl->cw_num[CC_id][harq_pid] == SINGLE_CW ? TX_DIVERSITY : LARGE_DELAY_CDD,   // transmission_scheme
                                           2,         // number of layers
		  ...
		  dl_req->number_pdu++;
                  ...
                  if(ue_sched_ctl->cw_num[CC_id][harq_pid] == MULTI_CW){
                      if(ue_sched_ctl->round[CC_id][harq_pid][oppose_tb] == 8){
                          ...
                          ue_sched_ctl->round[CC_id][harq_pid][oppose_tb] = 0;

                          dl_req->tl.tag = NFAPI_DL_CONFIG_REQUEST_BODY_TAG;

                          UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][oppose_tb] = 1 - UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][oppose_tb];
                          UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][oppose_tb] = UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[oppose_tb];

                          fill_nfapi_dlsch_config( &dl_req->dl_config_pdu_list[dl_req->number_pdu],
                                                   UE_info->eNB_UE_stats[CC_id][UE_id].TBS[oppose_tb],
                                                   eNB->pdu_index[CC_id],
                                                   rnti,
                                                   0, // type 0 allocation from 7.1.6 in 36.213
                                                   0, // virtual_resource_block_assignment_flag, unused here
                                                   0, // resource_block_coding, to be filled in later
                                                   getQm(UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[oppose_tb]),
                                                   0, // redundancy version
                                                   (oppose_tb + 1), // transport blocks
			  ...
			  dl_req->number_pdu++;
                          ...
                          eNB->TX_req[CC_id].sfn_sf = fill_nfapi_tx_req( &eNB->TX_req[CC_id].tx_request_body,
                                                                         ...
                                                                         UE_info->eNB_UE_stats[CC_id][UE_id].TBS[oppose_tb],
                                                                         eNB->pdu_index[CC_id],
                                                                         eNB->UE_info.DLSCH_pdu[CC_id][0][(unsigned char)UE_id].payload[0][oppose_tb]
                                                                       );
                          ...
                          fill_nfapi_dlsch_config( &dl_req->dl_config_pdu_list[dl_req->number_pdu],
                                                   ...
                                                   0,     // virtual_resource_block_assignment_flag, unused here
                                                   0,     // resource_block_coding, to be filled in later
                                                   getQm(UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][oppose_tb]),
                                                   UE_info->UE_sched_ctrl[UE_id].rsn[CC_id][harq_pid][oppose_tb] & 3, // redundancy version
                                                   (oppose_tb + 1),   // transport blocks
                                                   0,     // transport block to codeword swap flag
                                                   ue_sched_ctl->cw_num[CC_id][harq_pid] == SINGLE_CW ? TX_DIVERSITY : LARGE_DELAY_CDD,   // transmission_scheme
                                                   2,     // number of layers
          ...
          break;
            ...
            default:
              ...
              dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.tpc = 1;  // dont adjust power when retransmitting
              dl_config_pdu->dci_dl_pdu.
              dci_dl_pdu_rel8.new_data_indicator_1 = UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB1];
              dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.mcs_1 = UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB1];
              ...
              if (!CCE_allocation_infeasible
                ...
                fill_nfapi_dlsch_config(&dl_req->dl_config_pdu_list[dl_req->number_pdu],
                                        ...
                                        0,  // virtual_resource_block_assignment_flag, unused here
                                        0,  // resource_block_coding, to be filled in later
                                        getQm(UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB1]), round & 3, // redundancy version
                                        1,  // transport blocks
                                        0,  // transport block to codeword swap flag
          ...
          UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[TB1] = eNB_UE_stats->dlsch_mcs[TB1];
          UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[TB2] = eNB_UE_stats->dlsch_mcs[TB2];
        } else {
          LOG_D(MAC,
        ...
        TBS =
          get_TBS_DL(eNB_UE_stats->dlsch_mcs[select_tb], nb_available_rb);
        ...
        if (TBS - ta_len - header_len_dcch > 0) {
          ...
          dcch_size = rlc_status.bytes_in_buffer;
        ...
        if (TBS - ta_len - header_len_dcch - sdu_length_total -2 > 0) {
          ...
          dcch1_size = rlc_status.bytes_in_buffer;
        ...
        for (dtch = 0; dtch < dtch_max_num; dtch++) {
          ...
          if (TBS - ta_len - header_len_dcch - sdu_length_total - header_len_dtch > 0) {  // NN: > 2 ?
            ...
            dtch_size = rlc_status.bytes_in_buffer;
        ...
        if ((sdu_length_total + header_len_dcch +
          ...
          while (TBS <
            ...
            if (nb_rb > nb_available_rb) {  // if we've gone beyond the maximum number of RBs
              ...
              TBS =
                get_TBS_DL(eNB_UE_stats->dlsch_mcs[TB1],
                           nb_available_rb);
              nb_rb = nb_available_rb;
              break;
            }

            TBS = get_TBS_DL(eNB_UE_stats->dlsch_mcs[TB1], nb_rb);
          }
          ...
          LOG_D(MAC,
                ...
                eNB_UE_stats->dlsch_mcs[TB1], mcs);
#ifdef DEBUG_eNB_SCHEDULER
          LOG_D(MAC,
...
#endif
          offset = generate_dlsch_header((unsigned char *) UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][select_tb], num_sdus,  //num_sdus
                                         sdu_lengths, //
...
#endif
          // cycle through SDUs and place in dlsch_buffer
          memcpy(&UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][select_tb][offset],dlsch_buffer,sdu_length_total);
          // memcpy(RC.mac[0].DLSCH_pdu[0][0].payload[0][TB1][offset],dcch_buffer,sdu_lengths[0]);

          // fill remainder of DLSCH with random data
          for (j=0; j<(TBS-sdu_length_total-offset); j++) {
        	  UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][select_tb][offset+sdu_length_total+j] = 0;
          }

          if (opt_enabled == 1) {
            trace_pdu(DIRECTION_DOWNLINK, (uint8_t *)UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][select_tb],
          ...
          T(T_ENB_MAC_UE_DL_PDU_WITH_DATA, T_INT(module_idP), T_INT(CC_id), T_INT(rnti), T_INT(frameP), T_INT(subframeP),
            T_INT(harq_pid), T_BUFFER(UE_info->DLSCH_pdu[CC_id][0][UE_id].payload[0][TB1], TBS));
	  ...
	  UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[select_tb]=mcs;
          UE_info->eNB_UE_stats[CC_id][UE_id].TBS[select_tb] = TBS;
          UE_info->eNB_UE_stats[CC_id][UE_id].overhead_bytes[select_tb]= TBS- sdu_length_total;
          UE_info->eNB_UE_stats[CC_id][UE_id].total_sdu_bytes+= sdu_length_total;
          UE_info->eNB_UE_stats[CC_id][UE_id].total_pdu_bytes+= TBS;
          UE_info->eNB_UE_stats[CC_id][UE_id].total_num_pdus+=1;
          ...
          //dl_buffer_total set
          UE_info->UE_template[CC_id][UE_id].dl_buffer_total = dcch_size + dcch1_size + dtch_size;
...
//	  printf("dl sfn=%d, sf=%d, buffer=%d, TBS=%d\n", frameP, subframeP, UE_info->UE_template[CC_id][UE_id].dl_buffer_total, UE_info->eNB_UE_stats[CC_id][UE_id].TBS[select_tb]);
          ...
          if(   (first_TB_pdu_create_flg == 1)
             ...
             && (UE_info->UE_template[CC_id][UE_id].dl_buffer_total > UE_info->eNB_UE_stats[CC_id][UE_id].TBS[select_tb])){
            //2nd TB start
            ...
            //GET 1st TB TBS
            TBS = UE_info->eNB_UE_stats[CC_id][UE_id].TBS[select_tb];
            ...
            if ((sdu_length_total + header_len_dcch + header_len_dtch) > 0) {
                ...
                //GET 1st TB MCS and RBs
                mcs = UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[select_tb];
                nb_rb = UE_info->eNB_UE_stats[CC_id][UE_id].rbs_used;
        ...
        if(first_TB_pdu_create_flg == 1){
            ...
            switch (get_tmode(module_idP, CC_id, UE_id)) {
                ...
                case 3:
                    ...
                    if (!CCE_allocation_infeasible(module_idP,CC_id,1,subframeP,dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.aggregation_level,rnti)) {
                        //SET 1st TB
                        ue_sched_ctl->round[CC_id][harq_pid][select_tb] = 0;
                        dl_req->number_dci++;
                        dl_req->number_pdu++;
                        dl_req->tl.tag = NFAPI_DL_CONFIG_REQUEST_BODY_TAG;
                        ...
                        rnti,harq_pid,UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][select_tb]);

                        UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][select_tb] = 1 - UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][select_tb];
                        UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][select_tb] = UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[select_tb];
                        if(UE_info->UE_template[CC_id][UE_id].physicalConfigDedicated == NULL) {
                          LOG_E(MAC, "physicalConfigDedicated is NULL\n");
                          return;
                        }
                        if(UE_info->UE_template[CC_id][UE_id].physicalConfigDedicated->pdsch_ConfigDedicated == NULL) {
                        ...
                        fill_nfapi_dlsch_config( &dl_req->dl_config_pdu_list[dl_req->number_pdu],
                                                 ...
                                                 0, // virtual_resource_block_assignment_flag, unused here
                                                 0, // resource_block_coding, to be filled in later
                                                 getQm(UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[select_tb]),
                                                 0, // redundancy version
                                                 (select_tb + 1), // transport blocks
                                                 0, // transport block to codeword swap flag
                                                 ue_sched_ctl->cw_num[CC_id][harq_pid] == SINGLE_CW ? TX_DIVERSITY : LARGE_DELAY_CDD, // transmission_scheme
			...
			dl_req->number_pdu++;
                        eNB->TX_req[CC_id].sfn_sf = fill_nfapi_tx_req( &eNB->TX_req[CC_id].tx_request_body,
                                                                       ...
                                                                       UE_info->eNB_UE_stats[CC_id][UE_id].TBS[select_tb],
                                                                       eNB->pdu_index[CC_id],
                                                                       eNB->UE_info.DLSCH_pdu[CC_id][0][(unsigned char)UE_id].payload[0][select_tb]
                                                                     );
    ...
    if (UE_info->eNB_UE_stats[CC_id][UE_id].rrc_status == RRC_HO_EXECUTION) {
	  ...
	  if (!CCE_allocation_infeasible(module_idP,CC_id,1,subframeP,dl_config_pdu->dci_dl_pdu.dci_dl_pdu_rel8.aggregation_level,rnti)) {
	    ...
	    LOG_D(MAC,"CC_id %d Frame %d, subframeP %d: Toggling Format1 NDI for UE %d (rnti %x/%d) oldNDI %d\n",
		  CC_id, frameP,subframeP,UE_id,
		  rnti,harq_pid,UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB1]);
	    
        UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB1]=1-UE_info->UE_template[CC_id][UE_id].oldNDI[harq_pid][TB1];
        UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB1] = UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[TB1];
        UE_info->UE_template[CC_id][UE_id].oldmcs[harq_pid][TB2] = 0;
        if(UE_info->UE_template[CC_id][UE_id].physicalConfigDedicated == NULL) {
          LOG_E(MAC, "physicalConfigDedicated is NULL\n");
          return;
        }
	    ...
	    fill_nfapi_dlsch_config(&dl_req->dl_config_pdu_list[dl_req->number_pdu],
                    UE_info->eNB_UE_stats[CC_id][UE_id].TBS[TB1],
				    eNB->pdu_index[CC_id],
				    rnti,
				    0, // type 0 allocation from 7.1.6 in 36.213
				    0, // virtual_resource_block_assignment_flag, unused here
				    0, // resource_block_coding, to be filled in later
				    getQm(UE_info->eNB_UE_stats[CC_id][UE_id].dlsch_mcs[TB1]),
				    0, // redundancy version
				    1, // transport blocks
				    0, // transport block to codeword swap flag
				    cc[CC_id].p_eNB == 1 ? 0 : 1, // transmission_scheme
...
void ulsch_scheduler_pre_ue_select_fairRR(
  ...
  // Selection of Retx UEs, Preparation of VoLTE UE and Newtx UE List
  // UE round >0
  for ( UE_id = 0; UE_id < NUMBER_OF_UE_MAX; UE_id++ ) {
    if (UE_info->active[UE_id] == FALSE)
      continue;
    ...
    if (ulsch_ue_select[CC_id].ue_num < ulsch_ue_max_num_normal[CC_id]) {
  ...
  for ( UE_id_idx = 0; UE_id_idx < MAX_UE_MULTIPLEX; UE_id_idx++ ) {
    ...
    if( ( UE_sched_ctl->volte_configured == TRUE )
       ...
       && ( (ulsch_ue_select[CC_id].ue_num + selected_volte_ue_num) < ulsch_ue_max_num[CC_id] )
       && ( UE_sched_ctl->ul_periodic_timer_exp_flag == TRUE ) )
    {
      volte_lcg = UE_sched_ctl->volte_lcg;
      
      if ( UE_info->UE_template[CC_id][UE_id].ul_buffer_info[volte_lcg] > 0 )
      {
        hi_dci0_pdu   = &HI_DCI0_req->hi_dci0_pdu_list[HI_DCI0_req->number_of_dci+HI_DCI0_req->number_of_hi];
        format_flag = 2;
        ...
        if (CCE_allocation_infeasible(module_idP,CC_id,format_flag,subframeP,aggregation,rnti) == 1) {
          ...
          last_ulsch_ue_id_volte[CC_id] = UE_id;
          UE_sched_ctl->ul_periodic_timer_exp_flag = FALSE;
          continue;
        }
      }
    }
  ...
  for ( CC_id = 0; CC_id < MAX_NUM_CCs; CC_id++ ) {
    ...
    for ( ulsch_ue_num = 0; ulsch_ue_num < ulsch_ue_select[CC_id].ue_num; ulsch_ue_num++ ) {
      ...
      if ( ulsch_ue_select[CC_id].list[ulsch_ue_num].ue_priority == SCH_UL_RETRANS ) {
        ...
        if (mac_eNB_get_rrc_status(module_idP,UE_RNTI(module_idP, UE_id)) == RRC_HO_EXECUTION){
          UE_info->UE_template[CC_id][UE_id].pre_assigned_mcs_ul = 6;
        }
